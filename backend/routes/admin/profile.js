...
router.get("/", verifyAdmin, async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json(errorResponse("Failed to fetch admin profile", err.message));
  }
});
...
router.put("/", verifyAdmin, async (req, res) => {
  try {
    ...
    res.json(
      successResponse("Admin profile updated successfully", updatedAdmin),
    );
  } catch (err) {
    if (err.code === "P2002" && err.meta?.target) {
      ...
      return res
        .status(409)
        .json(errorResponse(`${conflictField} already in use`));
    }
    res
      .status(500)
      .json(errorResponse("Failed to update admin profile", err.message));
  }
});
