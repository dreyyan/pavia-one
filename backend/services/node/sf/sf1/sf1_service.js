...
// [IMPORT] Helpers
const { normalizeCol, splitName, parseParentName } = require("./utilities");

// [SERVICE] Generate students from SF1 template
function generateStudentsFromTemplate(sectionIdentifier) {
  ...
  if (!fs.existsSync(SF1_PATH))
    throw new Error("SF1.xlsx not found in forms folder");
  ...
  rawData.forEach((row, i) => {
    ...
    if (rowStr.includes("BARANGAY") && rowStr.includes("MUNICIPALITY"))
      subHeader = i;
    if (
      (rowStr.includes("FATHER") || rowStr.includes("MOTHER")) &&
      parentsHeader === -1
    )
      parentsHeader = i;
  ...
  if (mainHeader === -1 || subHeader === -1)
    throw new Error("Could not detect SF1 headers");
  ...
  if (parentsHeader >= 0)
    rawData[parentsHeader] = ffill(rawData[parentsHeader]);
  ...
  for (let c = 0; c < maxCols; c++) {
    ...
    if (
      parentsHeader >= 0 &&
      /(FATHER|MOTHER)/i.test(rawData[parentsHeader][c])
    )
      name = rawData[parentsHeader][c];
    if (remarksHeader >= 0 && /REMARK/i.test(rawData[remarksHeader][c]))
      name = rawData[remarksHeader][c];
  ...
  df.forEach((row) => {
    ...
    mergedHeaders.forEach(
      (colName, idx) => (student[colName] = row[idx] || ""),
    );
    ...
    if (sectionIdentifier && studentSection !== sectionIdentifier.toUpperCase())
      return;
    ...
    students.push({
      ...
      mother_tongue: (student["mother_tongue"] || "").replace(/\b\w/g, (l) =>
        l.toUpperCase(),
      ),
      religion: (student["religion"] || "").replace(/\b\w/g, (l) =>
        l.toUpperCase(),
      ),
      barangay: (student["barangay"] || "").replace(/\b\w/g, (l) =>
        l.toUpperCase(),
      ),
      municipality: (student["municipality"] || "Pavia").replace(/\b\w/g, (l) =>
        l.toUpperCase(),
      ),
      province: (student["province"] || "Iloilo").replace(/\b\w/g, (l) =>
        l.toUpperCase(),
      ),
      father_name: parseParentName(student["father_name"] || ""),
      mother_maiden_name: parseParentName(student["mother_maiden_name"] || ""),
      learning_modality: (student["learning_modality"] || "").replace(
        /\b\w/g,
        (l) => l.toUpperCase(),
      ),
...
function generateSF1PDFBuffer(section, students) {
  return new Promise((resolve, reject) => {
    try {
      ...
      doc
        .fontSize(16)
        .text(`SF1 - Section: ${section.name} Grade ${section.gradeLevel}`, {
          align: "center",
        });
      doc.moveDown(1);
